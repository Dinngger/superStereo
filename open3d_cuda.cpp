...
int32_t idx2id(const voxelIdx& idx) {
    Vector3i idxd(idx.elem.x, idx.elem.y, idx.elem.z);
    idxd -= ashift;
    if (idxd(0) < 0 || idxd(0) > 99 || idxd(1) < 0 || idxd(1) > 99 || idxd(2) < 0 || idxd(2) > 29)
        return -1;
    else
        return idxd(2) * 30 * 100 + idxd(1) * 100 + idxd(0);
}

std::map<voxelIdx, uint32_t> idxm;
int32_t idx[100*100*30] = {-1,};
...
int main(int argc, char** argv) {
    ...
    uint32_t cnt = 0;
    Vector3d map_center = Vector3d(-11283.1, -2601.05, 0);
    if (argc > 2) {
        for (int i=0; i<3; i++)
    ...
    while (true) {
        try {
            ...
            shift = _shift;
            ashift = ((map_center - shift) / 1.5).cast<int>() - Vector3i(50, 50, 15);
            for (int i=0; i<3; i++)
                f >> mean(i);
            mean -= map_center;
            for (int i=0; i<3; i++)
